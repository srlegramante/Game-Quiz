...
class MainPage:
    def __init__(self, root, names):
        ...
        self.status = customtkinter.CTkLabel(frame1, text="", text_color="black", width=200)
        self.status.pack(pady=20, padx=20)
    ...
    def answer_player(self):
        resposta = self.resposta_jogador.get().strip().lower()
        animal_correto = self.animal.strip().lower()
        if resposta == animal_correto:
            self.status.configure(text="Correto!", text_color="green")
        else:
            self.status.configure(text="Errado, tente novamente.", text_color="red")

        # Desabilitar o botão de dica enquanto uma resposta é avaliada
        self.btn_ver_dica.configure(state=customtkinter.NORMAL)

if __name__ == "__main__":
    root = customtkinter.CTk()
    app = MainPage(root, ["Player1", "Player2"])
    root.mainloop()
...
'''
        ...
        while not resposta_certa:
            ...
            if self.resposta_jogador == self.random_animal:
                ...
                print(f"O {player1} está com {self.quantidade_pontos_player1}. vamos para o proximo!")
            elif self.player1 != self.random_animal:
                print(f"Resposta errada... é a vez do {player2}.")
                self.btn_conferir.configure(state=customtkinter.NORMAL)
            elif self.resposta_jogador == self.random_animal:
                self.btn_ver_dica.configure(state=customtkinter.DISABLED)
                self.player2 = self.quantidade_pontos_player2 =+ 1
                print(f"O {player2} está com {self.quantidade_pontos_player2}. vamos para o proximo!")
            elif self.resposta_jogador != self.random_animal:
                print(f"Resposta errada... é a vez do {player1}.")
                self.btn_conferir.configure(state=customtkinter.NORMAL)
            else:
                print("Fim do jogo")
                resposta_certa = True
